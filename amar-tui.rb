...
def generate_town_ui
  ...
  begin
    ...
    last_progress = ""
    timeout_counter = 0
    max_timeout = 200  # 10 seconds (200 * 0.05)
    ...
    if thread.alive?
      thread.kill
      $stdout = original_stdout
      show_content("Error: Town generation timed out. Please try again.")
      return
    end
    ...
    loop do
      ...
      when "r"
        ...
        begin
          ...
          # Convert the output to use colors
          if @config[:color_mode]
            output = convert_ansi_to_rcurses(output)
          end
